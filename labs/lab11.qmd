...
The data I have included for you this week comes from Schweizer et al, 2015, and contains 123 individual sequences of grey wolves across north america. We first read in the genotypes, here formatted almost exactly like the `GT` matrices you have dealt with before, but just transposed (instead of each row being a variant, and each column being an individual, each row is an individual and each column is a variant).
...
Now we can run the `TESS3` analysis. Recall this is quite similar to `ADMIXTURE`, but includes a spatial component.
...
```{r}
area_plot = plot(ancestry_matrix,method="map.max",interpol=FieldsKrigModel(10))
```
...
Now, to actually get info on what is _at_ these significant (if any) SNPs, you'd need to get their locations out from the `.map` file that is produced by plink, the `.lfmm` format does not contain information about genetic location. These SNPs are ones that show significant differentiation between your groups - they are essentially identifiers of the different ancestries.